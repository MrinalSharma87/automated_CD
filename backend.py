    ...
    for tag, i1, i2, j1, j2 in sm.get_opcodes():
        if tag == 'insert':
            for para in paragraphs_v2[j1:j2]:
                changes.append({"type": "Added", "text": para})
        elif tag == 'delete':
            for para in paragraphs_v1[i1:i2]:
                changes.append({"type": "Deleted", "text": para})
        elif tag == 'replace':
            for old, new in zip(paragraphs_v1[i1:i2], paragraphs_v2[j1:j2]):
                changes.append({"type": "Modified", "old_text": old, "new_text": new})
    return changes
...
def analyze_change(change):
    try:
        ...
        print(f"Summarizing change: {input_text[:60]}...")
